...
LatLoads = sio.loadmat("C:\CODING\TireDataPY\Raw Tire DATA\R20 Runs\A2356raw12.mat")
def Convert(string):
...
df.to_csv('C:\\CODING\\TireDataPY\\Tire Analysis\\df.csv',sep = ',',index = False)
# creates the output file for the tire data to be used later
writer = pd.ExcelWriter('C:\CODING\TireDataPY\OUTPUT plots\R20testsdkfs.xlsx', engine = 'xlsxwriter')
...
LongLoads = sio.loadmat('C:\CODING\TireDataPY\Raw Tire DATA\LCO Runs\DriveBrake\A1464run30.mat')
def Convert(string):
    li = list(string.split(","))
    out = []
    for item in li:
...
dfLong.to_csv('C:\\CODING\\TireDataPY\\Tire Analysis\\df1.csv',sep = ',',index = False)
